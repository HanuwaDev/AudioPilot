...
- `Network`: runs when your PC connects to or disconnects from a network (supports Ethernet, WiFi, VPN). You can choose to trigger on connect, disconnect, or both. For connect or both, you specify a network name; for disconnect, it triggers on any network disconnection.
