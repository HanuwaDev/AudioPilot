...
- Run routines triggered by hotkeys, app startup, Steam Big Picture, device changes, or tray actions.
...
Routines save reusable output and input targets and can react to app startup, Steam Big Picture, device changes, hotkeys, or tray actions.
...
- Need share-safe diagnostics: use `AudioPilot.Cli.exe status --json --redact` or `AudioPilot.Cli.exe diagnostics status --json --redact`.
