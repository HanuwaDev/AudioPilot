...
public sealed class AppViewModelRoutineAppStartTests
{
    ...
    public async Task DeactivateRoutineStatefulSessionAsync_RemovesOlderSessionWithoutAffectingLatest()
    {
        ...
        activeSessions["app-start:routine-old:100"] = new AppViewModel.RoutineStatefulSession(
            "app-start:routine-old:100",
            "routine-old",
            "Old",
            ...
            rootProcessId: 100);
        activeSessions["app-start:routine-new:200"] = new AppViewModel.RoutineStatefulSession(
            "app-start:routine-new:200",
            "routine-new",
            "New",
            RoutineTriggerKind.AppStartup,
        ...
        await InvokeDeactivateRoutineStatefulSessionAsync(viewModel, "app-start:routine-old:100");

        Assert.DoesNotContain("app-start:routine-old:100", activeSessions.Keys);
        Assert.Contains("app-start:routine-new:200", activeSessions.Keys);
    }
    ...
    public void GetInvalidRoutineStatefulSessionKeys_ReturnsNewestInvalidSessionsFirst()
    {
        ...
        {
            ["app-start:routine-a:100"] = new(
                "app-start:routine-a:100",
                "routine-a",
                "Routine A",
                RoutineTriggerKind.AppStartup,
                activationSequence: 1,
            ...
            ["steam-big-picture:routine-b"] = new(
                ...
                restorePreviousAudioOnDeactivate: true,
                restoreSnapshot: null),
            ["app-start:routine-c:200"] = new(
                "app-start:routine-c:200",
                "routine-c",
                "Routine C",
                RoutineTriggerKind.AppStartup,
                activationSequence: 2,
        ...
        Assert.Equal(["steam-big-picture:routine-b", "app-start:routine-a:100"], result);
    ...
    public void BuildRoutineExecutionLogContext_IncludesStableExecutionFields()
    {
        ...
        string context = AppViewModel.BuildRoutineExecutionLogContext(routine, "hotkey", showOverlay: true, appStartProcessId: null);
        ...
        Assert.Contains("appStartProcessId=none", context);
    ...
    [Fact]
    public void BuildRoutineExecutionLogContext_RedactsAppStartProcessId()
    {
        var routine = new AudioRoutine
        {
            Id = "routine-1",
        ...
        string context = AppViewModel.BuildRoutineExecutionLogContext(routine, "app-start", showOverlay: true, appStartProcessId: 321);

        Assert.Contains("appStartProcessId=id[len=3 hash=", context, StringComparison.Ordinal);
        Assert.DoesNotContain("appStartProcessId=321", context, StringComparison.Ordinal);
    }

    [Fact]
    public void BuildRoutineExecutionResultLogContext_IncludesOutcomeFlagsAndNames()
    {
    ...
    public void BuildRoutineStatefulSessionLogContext_IncludesRestoreDecision()
    {
        ...
        Assert.Contains($"sessionKey={LogPrivacy.Session("app-start:routine-1:321")}", context);
        Assert.Contains($"routineId={LogPrivacy.Id("routine-1")}", context);
        Assert.Contains($"routineName={LogPrivacy.Label("Desk")}", context);
        Assert.Contains("triggerKind=AppStartup", context);
    ...
    public void LogRoutineAppStartMatchSkipped_IncludesCorrelatedOperationId()
    {
        ...
        Assert.Contains("routine-app-start-match-skipped", logText, StringComparison.Ordinal);
        ...
        Assert.Contains("appStartProcessId=id[len=3 hash=", logText, StringComparison.Ordinal);
        ...
        Assert.DoesNotContain(AppConstants.Audio.LogEvents.ViewModel.App.RoutineAppStartBatch, logText, StringComparison.Ordinal);
    }
