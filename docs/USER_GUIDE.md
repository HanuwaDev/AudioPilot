...
Use this when you want AudioPilot to react automatically when a desktop app or packaged app starts.

1. Create a routine and choose **App startup** as the primary trigger.
...
App-start matching uses the saved target, not just the file name. If the routine does not trigger, verify the exact app path or packaged-app identifier first.
...
Routines save a named output target, input target, or both, and let you trigger that saved setup in one step.

### Routine Trigger Modes

- `Hotkey`: runs when you press the routine hotkey.
- `App startup`: runs when AudioPilot detects a matching desktop app or packaged app starting.
...
- an app-start workflow that should restore speakers after the app exits.

### Packaged Apps

App-start routines support both normal desktop `.exe` targets and packaged apps such as Microsoft Store or MSIX-style apps.
...
- primary: app directory as `settings.json` when writable,
- fallback: `%AppData%/AudioPilot/settings.json`.
...
### App-Start Routine Does Not Trigger

1. Confirm the routine is enabled.
2. Verify the exact desktop path or packaged-app target.
3. Make sure the target app starts after AudioPilot is already running.
...
1. Check whether the app is using the `%AppData%/AudioPilot` fallback path.
2. Backup the current file before editing anything.
3. Prefer UI or CLI reconfiguration before manual JSON edits.
4. If the file is unreadable, let the app attempt backup recovery first.
...
3. Include `AudioPilot.Cli.exe status --json --redact`.
4. If the issue involves refresh, backups, or reconnect behavior, include `AudioPilot.Cli.exe diagnostics status --json --redact`.
5. Attach the relevant part of `AudioPilot.log` if you are comfortable sharing it.
