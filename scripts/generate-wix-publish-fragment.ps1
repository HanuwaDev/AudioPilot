...
Write-Indent -Builder $builder -Level 4 -Line '<RemoveFile Id="rmfInstallFolderFiles" Name="*" On="uninstall" />'
...
foreach ($directoryPath in ($relativeDirectories | Sort-Object)) {
    ...
    Write-Indent -Builder $builder -Level 4 -Line ('<RemoveFile Id="{0}" Name="*" On="uninstall" />' -f (Get-StableId -Prefix "rmf" -Value $directoryPath))
