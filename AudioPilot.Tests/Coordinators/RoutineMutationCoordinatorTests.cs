...
public sealed class RoutineMutationCoordinatorTests
{
    ...
    [Fact]
    public void Create_RejectsVolumeOnlyRoutine()
    {
        Settings settings = new();
        ...
        Assert.Equal("routine-invalid", result.ErrorCode);
        Assert.Contains("must target at least one output device or input device", result.Message, StringComparison.OrdinalIgnoreCase);
    }
