...
namespace AudioPilot.ViewModels
{
    public partial class AppViewModel
    {
        ...
        private readonly ISteamBigPictureSignalMonitor _steamBigPictureSignalMonitor;
        ...
        private void RegisterRoutineStatefulSession(AudioRoutine routine, int? rootProcessId, RoutineAudioRestoreSnapshot? restoreSnapshot)
        {
            ...
            _logger.Info(
                "AppViewModel",
                () => $"routine-stateful-session-registered | {BuildRoutineStatefulSessionLogContext(session, shouldRestore: null)} hasRestoreSnapshot={restoreSnapshot.HasValue}");
        ...
        private async Task RestoreRoutineAudioSnapshotAsync(RoutineStatefulSession session)
        {
            ...
            {
                await AppViewModelRoutineRestoreHelper.RestoreAsync(
                    session,
                    default,
                    _logger);
                return;
        ...
        internal async Task ExecuteDeviceChangeTriggeredRoutinesAsync(CancellationToken cancellationToken)
        {
            ...
            foreach (AudioRoutine routine in routines)
            {
                ...
                if (!result.Success)
                {
                    _logger.Warning(
                        "AppViewModel",
                        $"routine-device-change-trigger-failed | routineId={FormatRoutineLogIdentifier(routine.Id)}");
                }
            }
        }
        ...
        private Task<RoutineExecutionResult> ExecuteRoutineOnDispatcherAsync(AudioRoutine routine, bool showOverlay, int? appStartProcessId, string executionSource, CancellationToken cancellationToken = default)
        {
            if (_dispatcher.CheckAccess())
            {
                return ExecuteRoutineAsync(routine, showOverlay, appStartProcessId, executionSource, cancellationToken: cancellationToken);
            }

            return InvokeOnDispatcherAsync(
                () => ExecuteRoutineAsync(routine, showOverlay, appStartProcessId, executionSource, cancellationToken: cancellationToken),
        ...
        private void UpdateSteamBigPictureMonitorState()
        {
            ...
            if (stopMonitor)
            {
                _steamBigPictureSignalMonitor.Stop();
            ...
            SteamBigPictureSignalMonitorStartResult startResult = _steamBigPictureSignalMonitor.Start();
            if (startResult.Success)
            {
                _logger.Info("AppViewModel", () => $"steam-big-picture-monitor-active | watcher={_steamBigPictureSignalMonitor.GetType().Name}");
            ...
            _logger.Warning("AppViewModel", () => $"steam-big-picture-monitor-fallback-active | reason={startResult.FailureReason ?? startResult.Status} mode=event-revalidation watcher={_steamBigPictureSignalMonitor.GetType().Name}");
            QueueSteamBigPictureSignalEvaluation();
        }
