...
namespace AudioPilot.Platform
{
    ...
    public class SingleInstanceHelper : IDisposable
    {
        ...
        internal SingleInstanceHelper(string mutexName, string activationPipeName)
        {
        ...
        private void StopActivationListener()
        {
            ...
            try
            {
                ...
                {
                    bool stopped = listenerTask.Wait(TimeSpan.FromSeconds(AppConstants.Timing.SingleInstanceListenerStopTimeoutSeconds));
                    if (!stopped)
                    {
        ...
        private async Task ActivationListenerLoopAsync(CancellationToken cancellationToken)
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    string? payload = await ReadFramedMessageAsync(server, cancellationToken);
                    ...
                    if (string.IsNullOrWhiteSpace(payload))
                    {
                        if (_logger.IsEnabled(LogLevel.Trace))
                        {
                            _logger.Trace("SingleInstanceHelper", () => $"{AppConstants.Audio.LogEvents.SingleInstance.ActivationSignalIgnored} | reason=whitespace-payload");
                        }
                        continue;
                    }
        ...
        private bool SignalExistingInstance(string payloadText)
        {
            try
            {
                using var client = new NamedPipeClientStream(".", _activationPipeName, PipeDirection.InOut, PipeOptions.None);
                client.Connect(AppConstants.Timing.SingleInstanceConnectTimeoutMs);
                WriteFramedMessage(client, payloadText);

                if (!TryReadResponse(client, out SingleInstanceCommandResult response, out string normalizationMode))
                {
        ...
        private static bool TryReadResponse(NamedPipeClientStream client, out SingleInstanceCommandResult response, out string normalizationMode)
        {
            response = default;
            normalizationMode = string.Empty;

            if (!TryReadFramedMessage(client, out string responseText))
            {
                return false;
            }
        ...
        private static async Task WriteFramedMessageAsync(NamedPipeServerStream server, string payload, CancellationToken cancellationToken)
        {
            var bytes = Encoding.UTF8.GetBytes(payload);
            var lengthBytes = BitConverter.GetBytes(bytes.Length);
            await server.WriteAsync(lengthBytes.AsMemory(0, lengthBytes.Length), cancellationToken);
            await server.WriteAsync(bytes.AsMemory(0, bytes.Length), cancellationToken);
            await server.FlushAsync(cancellationToken);
        }

        private static async Task<string?> ReadFramedMessageAsync(NamedPipeServerStream server, CancellationToken cancellationToken)
        {
            var lengthBytes = new byte[sizeof(int)];
            if (!await ReadExactlyAsync(server, lengthBytes, cancellationToken))
            {
                return null;
            }
            ...
            if (!await ReadExactlyAsync(server, payloadBytes, cancellationToken))
            {
                return null;
            }
        ...
        private static bool TryReadFramedMessage(NamedPipeClientStream client, out string payload)
        {
            payload = string.Empty;

            var lengthBytes = new byte[sizeof(int)];
            if (!ReadExactly(client, lengthBytes))
            {
                return false;
            }
            ...
            var payloadBytes = new byte[length];
            if (!ReadExactly(client, payloadBytes))
            {
                return false;
            }
