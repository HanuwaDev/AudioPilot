...
namespace AudioPilot.Services.Bluetooth
{
    ...
    public sealed class BluetoothReconnectCoordinator(IBluetoothReconnectService reconnectService, Logger logger)
    {
        ...
        private sealed class TimeoutCircuitState
        {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            bool relaxedSingleDisconnectedCandidate = false;
            ...
            _logger.Info("BluetoothReconnect", () => $"{AppConstants.Audio.LogEvents.BluetoothReconnect.Start} | opId={opId} kind={ToKind(deviceKind)} disconnected={disconnected.Count} eligible={eligible.Count} maxAttempts={options.MaxAttempts} timeoutMs={options.AttemptTimeoutMs} relaxedSingleCandidate={relaxedSingleDisconnectedCandidate}");
                ...
                while (attempts < options.MaxAttempts)
                {
                    ...
                    catch (OperationCanceledException) when (!cancellationToken.IsCancellationRequested)
                    {
                        ...
                        if (fallbackAttempt.Connected)
                        {
                            ...
                            LogAttemptPhases(opId, deviceKind, attempts, pairMs, fallbackMs, reconnectStopwatch.Elapsed.TotalMilliseconds - attemptStartMs, phaseResult, phaseSource, relaxedSingleDisconnectedCandidate);
        ...
        private void LogAttemptPhases(
            ...
            string result,
            string source,
            bool relaxedSingleDisconnectedCandidate)
        {
            if (!_logger.IsEnabled(LogLevel.Debug))
            {
                return;
            ...
            _logger.Debug(
                "BluetoothReconnect",
                () => $"{AppConstants.Audio.LogEvents.BluetoothReconnect.Phases} | opId={opId} kind={ToKind(deviceKind)} attempt={attempt} pairMs={pairMs:F1} fallbackMs={fallbackMs:F1} totalMs={totalMs:F1} result={result} source={source} relaxedSingleCandidate={relaxedSingleDisconnectedCandidate}");
        }
        ...
        private TimeoutCircuitUpdate RegisterTimeout(string deviceId, DateTime nowUtc)
        {
            TimeoutCircuitState state = _timeoutCircuitByDeviceId.AddOrUpdate(
                deviceId,
                _ => new TimeoutCircuitState { ConsecutiveTimeouts = 1, OpenUntilUtc = DateTime.MinValue },
                (_, existing) =>
                {
                    existing.ConsecutiveTimeouts++;
                    return existing;
                });

            bool openedCircuit = false;
            if (state.ConsecutiveTimeouts >= RuntimeTuningConfig.BluetoothReconnectTimeoutCircuitThreshold)
            {
                state.OpenUntilUtc = nowUtc.AddMilliseconds(RuntimeTuningConfig.BluetoothReconnectTimeoutCircuitOpenMs);
                openedCircuit = true;
            }
