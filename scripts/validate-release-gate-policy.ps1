...
Assert-Pattern -Pattern 'AUDIOPILOT_RUN_STRESS:\s*"1"' -Description 'stress gate must enable AUDIOPILOT_RUN_STRESS.'

$requiredFilterPatterns = @(
    'dotnet test AudioPilot\.sln --configuration Release --no-build --verbosity normal --filter "Category!=Integration&Category!=Stress"',
    'dotnet test AudioPilot\.sln --configuration Release --no-build --verbosity normal --filter "Category=Integration"',
    'dotnet test AudioPilot\.sln --configuration Release --no-build --verbosity normal --filter "Category=Stress"'
)
