...
namespace AudioPilot.Coordinators
{
        ...
        Func<bool>? mediaPreviousTrackCommand = null)
    {
        ...
        private readonly Func<bool> _mediaPlayPauseCommand = mediaPlayPauseCommand ?? MediaKeyHelper.TryPressPlayPause;
        private readonly Func<bool> _mediaNextTrackCommand = mediaNextTrackCommand ?? MediaKeyHelper.TryPressNextTrack;
        private readonly Func<bool> _mediaPreviousTrackCommand = mediaPreviousTrackCommand ?? MediaKeyHelper.TryPressPreviousTrack;

        internal bool IsMediaOverlayCaptureInFlightForTests => Volatile.Read(ref _mediaOverlayCaptureInFlight) != 0;

        public void MediaPlayPause()
        {
            StartMediaCommand(MediaOverlayCommand.PlayPause, _mediaPlayPauseCommand);
        }

        public void MediaNextTrack()
        {
            StartMediaCommand(MediaOverlayCommand.NextTrack, _mediaNextTrackCommand);
        }

        public void MediaPreviousTrack()
        {
            StartMediaCommand(MediaOverlayCommand.PreviousTrack, _mediaPreviousTrackCommand);
        }
        ...
        private void StartMediaCommand(MediaOverlayCommand command, Func<bool> sendCommand)
        {
            int requestVersion = Interlocked.Increment(ref _latestMediaOverlayRequestVersion);
            if (Interlocked.CompareExchange(ref _mediaOverlayCaptureInFlight, 1, 0) != 0)
            {
                if (!MediaOverlayEngine.TrySendCommand(sendCommand, command))
                {
                    ApplyMediaOverlayResult(MediaOverlayEngine.BuildCommandSendFailureResult(command));
                    return;
                }

                if (logger.IsEnabled(LogLevel.Trace))
                {
                    logger.Trace("AppCliOverlayCoordinator", () => $"media-overlay-capture-skipped | command={command} reason=capture-in-flight");
                }

                return;
            }

            _ = ShowMediaOverlayAsync(command, sendCommand, requestVersion);
        }

        private async Task ShowMediaOverlayAsync(MediaOverlayCommand command, Func<bool> sendCommand, int requestVersion)
        {
            try
            {
                MediaOverlayResult mediaOverlay = await mediaOverlayCommands.SendWithBestEffortOverlayAsync(command, sendCommand);
