            ...
            out string? error)
        {
            ...
            try
            {
                ...
                if (!verifiedEnabled || !verifiedRenderTarget)
                {
                    ...
                    _logger.Info(
                        "AudioDeviceService",
                        () => $"{AppConstants.Audio.LogEvents.Listen.SetSuccess} | target={enabled} verified=unknown device={LogPrivacy.Device(inputDeviceName)} id={LogPrivacy.Id(inputDeviceId)}");
                    return true;
                }
