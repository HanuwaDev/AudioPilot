...
namespace AudioPilot.Cli
{
    ...
    public sealed partial class CliCommand
    {
        ...
        private static bool TryParseNetworkCommand(string[] tokens, string[] args, out CliCommand command, out string? error)
        {
            ...
            if (tokens.Length < 2)
            {
                error = $"Missing network command. Use: network list";
                return false;
            }

            if (tokens[1] == "list")
            {
                command = new CliCommand { Action = CliAction.NetworkList };
                return true;
            }

            error = CliSuggestionHelper.BuildUnknownValueError("network command", args[1], ["list"], "network list");
            return false;
        }
