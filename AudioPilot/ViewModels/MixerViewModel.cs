...
namespace AudioPilot.ViewModels
{
        ...
        DeviceCacheHelper? deviceCache = null)
    {
        ...
        private readonly ConcurrentBag<List<(string Id, float Volume)>> _toUpdatePool = [];
        ...
        private AudioSessionItem CreateSessionItem(string sessionId, AudioSessionSnapshot session, uint? processId)
        {
            ...
            if (IsSharedSessionId(sessionId) && _sharedSessionBridge is SharedSessionBridge bridge)
            {
                return bridge.GetOrAdd(
                    sessionId,
                    () => new AudioSessionItem(displayName, session.Volume, isMaster, isMic, isSystemSounds, processId));
            }

            return new AudioSessionItem(displayName, session.Volume, isMaster, isMic, isSystemSounds, processId);
        }
        ...
        private List<(string Id, float Volume)> RentToUpdateList()
        {
        ...
        private void ReturnToUpdateList(List<(string Id, float Volume)>? list)
        {
            if (list == null)
            {
                return;
        ...
        public async Task RefreshAsync(bool interactive = true)
        {
            ...
            List<(string Id, float Volume)>? toUpdate = null;
            ...
            try
            {
                ...
                foreach (var session in serviceSessions)
                {
                    ...
                    if (!weJustSetIt && Math.Abs(currentVolume - systemVolume) > 0.1f)
                    {
                        toUpdate ??= RentToUpdateList();
                        toUpdate.Add((id, systemVolume));
        ...
        internal async Task<bool> ApplyRefreshResultsAsync(
            ...
            List<(string Id, float Volume)>? toUpdate,
            CancellationToken cancellationToken)
        {
            ...
            try
            {
                await _dispatcher.InvokeAsync(() =>
                {
                    ...
                    if (toUpdate != null)
                    {
                        foreach (var (id, volume) in toUpdate)
                        {
        ...
        private void ClearCollectionPools()
        {
            ...
            while (_toUpdatePool.TryTake(out List<(string Id, float Volume)>? toUpdate))
            {
                toUpdate.Clear();
            }
        ...
        private async Task ObserveVolumeApplyAsync(Task task, string phase)
        {
            try
            {
                await task;
