            ...
            foreach (AppViewModel.RoutineAppOutputLease lease in activeLeases)
            {
                ...
                foreach (uint processId in candidateProcessIds)
                {
                    ...
                    if (outputAppliedForProcess || inputAppliedForProcess)
                    {
                        logger.Info(
                            "AppViewModel",
                            () => $"routine-app-start-lease-process-applied | {AppViewModel.BuildRoutineAppOutputLeaseLogContext(lease)} processId={AppViewModel.FormatRoutineLogProcessId((int)processId)} outputApplied={outputAppliedForProcess} inputApplied={inputAppliedForProcess}");
                    }
                    ...
                    if (!lease.CompletionOverlayShown && HasLeaseCompleted(lease) && (outputAppliedForProcess || inputAppliedForProcess))
                    {
                        ...
                        logger.Info(
                            "AppViewModel",
                            () => $"routine-app-start-lease-completed | {AppViewModel.BuildRoutineAppOutputLeaseLogContext(lease)}");
                    }
                }
            }
        }
