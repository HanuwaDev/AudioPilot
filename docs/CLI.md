...
AudioPilot.Cli.exe startup enable|disable|status [--json] [--redact]
AudioPilot.Cli.exe startup open
AudioPilot.Cli.exe help [completion|diagnostics|media|mute|listen|volume|routine|config|runtime|network|devices|cycle|switch|wait|startup]
...
- **Miscellaneous**: BluetoothReconnectEnabled, **PreserveAudioLevels**, DeviceReferenceFileMode, LogLevel, RedactLogContent, AutoSaveEnabled, ScheduleTimeZoneId, AutoScrollToMixerOnRestore, SuppressDeviceStartupWarnings
...
`theme`, `auto-save-enabled`, `run-at-startup`, `preserve-audio-levels`, `auto-scroll-to-mixer-on-restore`, `suppress-device-startup-warnings`, `generate-device-reference-file`, `schedule-timezone` (Windows time zone ID, e.g., `Pacific Standard Time` or `UTC`)
