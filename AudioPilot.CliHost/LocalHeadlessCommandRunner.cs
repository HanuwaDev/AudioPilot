...
namespace AudioPilot.CliHost
{
    internal sealed class LocalHeadlessCommandRunner : IDisposable, ICliCommandRuntime
    {
        ...
        private sealed class LazyRuntimeServices : IDisposable
        {
            ...
            public static LazyRuntimeServices CreateFromBundle(AppRuntimeServiceBundle runtimeServices)
            {
                ...
                return new LazyRuntimeServices(
                    ...
                    () => runtimeServices.BluetoothReconnectCoordinator,
        ...
        {
            MediaKeyHelper.PressPlayPause();
            RecordCliActionHistory(ExecutionHistoryKind.Media, "media-play-pause", success: true, skipped: false, summary: "Dispatched Play/Pause media command.");
        }

        public void MediaNextTrack()
        {
            MediaKeyHelper.PressNextTrack();
            RecordCliActionHistory(ExecutionHistoryKind.Media, "media-next-track", success: true, skipped: false, summary: "Dispatched Next Track media command.");
        }

        public void MediaPreviousTrack()
        {
            MediaKeyHelper.PressPreviousTrack();
            RecordCliActionHistory(ExecutionHistoryKind.Media, "media-previous-track", success: true, skipped: false, summary: "Dispatched Previous Track media command.");
        }
        ...
        public (bool IsValid, string Output) GetConfigValidation(bool jsonOutput)
        {
            Settings settings = SettingsService.LoadSettings();
            SettingsDiagnosticsResult diagnostics = SettingsValidationService.EvaluateDiagnostics(
                settings,
            ...
            return (
                warnings.Count == 0,
                CliOutputFormatter.FormatConfigValidation(warnings, jsonOutput));
        }
        ...
        {
            return _audioOverrides?.GetLogRootDirectory?.Invoke() ?? AppDomain.CurrentDomain.BaseDirectory;
        }
