...
namespace AudioPilot.ViewModels
{
    public partial class AppViewModel
    {
            ...
            DeviceCacheHelper? deviceCache = null)
        {
            ...
            _executionHistory = executionHistory ?? new ExecutionHistoryService();
            ...
            _steamBigPictureSignalMonitor = steamBigPictureSignalMonitor ?? new WinEventSteamBigPictureSignalMonitor();
            _steamBigPictureSignalMonitor.Signaled += OnSteamBigPictureMonitorSignaled;
            ...
            SettingsDeafenHotkeyDraftCapture.BaseHoverText = "Toggles deafen (mute mic and sound together). Press Delete to clear.";
            SettingsListenToInputHotkeyDraftCapture.BaseHoverText = "Toggles Windows \"Listen to this device\" input monitoring for the current default input endpoint. Press Delete to clear.";
            SettingsMasterVolumeUpHotkeyDraftCapture.BaseHoverText = "Raises default output master volume by the configured step. Press Delete to clear.";
            SettingsMasterVolumeDownHotkeyDraftCapture.BaseHoverText = "Lowers default output master volume by the configured step. Press Delete to clear.";
            SettingsMicVolumeUpHotkeyDraftCapture.BaseHoverText = "Raises default microphone volume by the configured step. Press Delete to clear.";
            SettingsMicVolumeDownHotkeyDraftCapture.BaseHoverText = "Lowers default microphone volume by the configured step. Press Delete to clear.";
            ...
            var refreshCommand = TrackCommand(new RelayCommand(async () => await RefreshDevicesAsync(), () => !IsRefreshing), observeViewModel: true);
            var saveCommand = TrackCommand(new RelayCommand(async () => await SaveSettingsAsync(), () => !_isSaving), observeViewModel: true);
            var saveCurrentContextCommand = TrackCommand(new RelayCommand(async () => await SaveCurrentContextAsync(), () => !_isSaving && !_isApplyingSettings && !IsSavingRoutines), observeViewModel: true);
            var applySettingsCommand = TrackCommand(new RelayCommand(async () => await ApplySettingsAsync(), () => !_isApplyingSettings && !_isSaving), observeViewModel: true);
            var importSettingsCommand = TrackCommand(new RelayCommand(async () => await ImportSettingsAsync(), () => !_isApplyingSettings && !_isSaving), observeViewModel: true);
            var exportSettingsCommand = TrackCommand(new RelayCommand(async () => await ExportSettingsAsync(), () => !_isApplyingSettings && !_isSaving), observeViewModel: true);
            var resetPerAppAudioRoutingCommand = TrackCommand(new RelayCommand(async () => await ResetPerAppAudioRoutingAsync(), () => !_isApplyingSettings && !_isSaving), observeViewModel: true);
            var resetCommand = TrackCommand(new RelayCommand(async () => await ResetToDefaultsAsync()));
            ...
            var saveRoutinesCommand = TrackCommand(new RelayCommand(async () => await SaveRoutinesFromButtonAsync(), () => !IsSavingRoutines && (HasRoutines || HasUnsavedRoutineChanges)), observeViewModel: true);
