...
public sealed partial class MainWindowStartupResumeCoordinatorTests
{
    ...
    [Fact]
    public async Task HandlePowerModeChanged_LogsSkip_WhenRecoveryIsAlreadyInProgress()
    {
        ...
        coordinator.HandlePowerModeChanged(new PowerModeChangedEventArgs(PowerModes.Resume), nameof(HandlePowerModeChanged_LogsSkip_WhenRecoveryIsAlreadyInProgress));
        ...
        string skipLogText = TestLogFileAssert.WaitForLogText(loggerScope.LogPath, AppConstants.Audio.LogEvents.ResumeRecovery.Skip);
        recoveryHandler.Release();
        await recoveryHandler.WaitForCompletionAsync();

        Assert.Equal(1, recoveryHandler.InvocationCount);
        Assert.Contains($"{AppConstants.Audio.LogEvents.ResumeRecovery.Skip} | opId=", skipLogText, StringComparison.Ordinal);
        Assert.Contains("reason=recovery-in-progress", skipLogText, StringComparison.Ordinal);
        Assert.Contains($"power-resume-detected | opId={opId}", skipLogText, StringComparison.Ordinal);
    }
