...
public sealed class AppCliOverlayCoordinatorTests
{
    ...
    public async Task MediaNextTrack_WhenCaptureAlreadyInFlight_SendsCommandAndSchedulesTrailingCapture()
    {
        ...
        await AssertEventuallyAsync(
            () => Volatile.Read(ref sendCount) == 1 && presenter.MessageUpdateCount == 1,
            GetMediaOverlayCaptureTimeout());
        Assert.Contains(presenter.Messages, message => string.Equals(message.header, "Next track", StringComparison.Ordinal));
    ...
    [Fact]
    public async Task MediaNextTrack_WhenNewerPressArrives_ShowsAcknowledgementAndTrailingOverlay()
    {
        var presenter = new RecordingOverlayPresenter();
        var history = new ConcurrentQueue<ExecutionHistoryEntry>();
        int snapshotCallCount = 0;
        ...
        await AssertEventuallyAsync(
            () => Volatile.Read(ref sendCount) == 1 && presenter.MessageUpdateCount == 1,
            GetMediaOverlayCaptureTimeout());
        Assert.Contains(presenter.Messages, message => string.Equals(message.header, "Play/pause command sent", StringComparison.Ordinal));
