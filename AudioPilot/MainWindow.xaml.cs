...
namespace AudioPilot
{
    ...
    internal readonly record struct MainWindowUiServices(
        ...
        Lazy<MediaOverlayCommandService> MediaOverlayCommands);
    ...
    public partial class MainWindow : Window
    {
        ...
        private readonly Lazy<OverlayService> _overlayService;
        private readonly Lazy<MediaOverlayCommandService> _mediaOverlayCommands;
        ...
        private MainWindowUiServices CreateUiServices()
        {
            return new MainWindowUiServices(
                ...
                new Lazy<OverlayService>(() => new OverlayService(), System.Threading.LazyThreadSafetyMode.ExecutionAndPublication),
                new Lazy<MediaOverlayCommandService>(() => new MediaOverlayCommandService(), System.Threading.LazyThreadSafetyMode.ExecutionAndPublication));
        }
        ...
        private MainWindowComposition CreateMainWindowComposition()
        {
            ...
            var cliOverlayCoordinator = new AppCliOverlayCoordinator(
                _audioService,
                _overlayService.Value,
                _mediaOverlayCommands.Value,
                _logger,
                () => appViewModel?.CurrentSettings,
                mediaHistoryRecorder: entry => appViewModel?.RecordCoordinatorExecutionHistory(entry));
            var switchCoordinator = new AppSwitchCommandCoordinator(
                _audioService,
                _overlayService.Value,
                _logger,
                _runtimeServices.BluetoothReconnectCoordinator.Value,
            ...
            AppViewModel composedAppViewModel = appViewModel = new AppViewModel(
                ...
                overlay: _overlayService.Value,
                dispatcher: Dispatcher,
                routineBluetoothReconnectCoordinator: _runtimeServices.BluetoothReconnectCoordinator.Value,
        ...
        private MainWindowHotkeyBindings CreateHotkeyBindings(AppViewModel appViewModel)
        {
            return new MainWindowHotkeyBindings(
                ...
                onMediaPlayPause: () => RunBackgroundHotkeyAction(
                    () => appViewModel.MediaPlayPauseFromHotkey(),
                    "Play/pause hotkey action error",
                    nameof(appViewModel.MediaPlayPauseFromHotkey)),
                onMediaNextTrack: () => RunBackgroundHotkeyAction(
                    () => appViewModel.MediaNextTrackFromHotkey(),
                    "Next track hotkey action error",
                    nameof(appViewModel.MediaNextTrackFromHotkey)),
                onMediaPreviousTrack: () => RunBackgroundHotkeyAction(
                    () => appViewModel.MediaPreviousTrackFromHotkey(),
                    "Previous track hotkey action error",
                    nameof(appViewModel.MediaPreviousTrackFromHotkey)),
        ...
        private void RepoLinkBorder_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Space || e.Key == Key.Enter)
            {
                ...
                {
                    _logger.Warning("MainWindow", () => $"Failed to open repository link: {ex.GetType().Name}", nameof(RepoLinkBorder_KeyDown), ex);
                }
        ...
        private void MainWindow_IsVisibleChanged(object sender, DependencyPropertyChangedEventArgs e)
        {
            ...
            _visibilityCoordinator.HandleVisibleChanged(
                ...
                isAutoScrollEnabled: () => _appVm.Settings!.Miscellaneous.AutoScrollToMixerOnRestore,
        ...
        private MainWindowShutdownDependencies CreateShutdownDependencies()
        {
            return MainWindowShutdownDependencyFactory.Build(
                ...
                () => { if (_overlayService.IsValueCreated) { _overlayService.Value.Dispose(); } },
                () => _shell?.Dispose(),
