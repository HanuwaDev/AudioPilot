...
namespace AudioPilot.Coordinators
{
        ...
        Func<Task<MediaKeyHelper.MediaCommandSendOutcome>>? mediaPreviousTrackCommandDetailedAsync = null)
    {
        ...
        private readonly Func<Task<MediaKeyHelper.MediaCommandSendOutcome>> _mediaPlayPauseCommand = ResolveMediaCommand(mediaPlayPauseCommandDetailedAsync, mediaPlayPauseCommandAsync, mediaPlayPauseCommand, MediaKeyHelper.TryPressPlayPauseDetailedAsync);
        private readonly Func<Task<MediaKeyHelper.MediaCommandSendOutcome>> _mediaNextTrackCommand = ResolveMediaCommand(mediaNextTrackCommandDetailedAsync, mediaNextTrackCommandAsync, mediaNextTrackCommand, MediaKeyHelper.TryPressNextTrackDetailedAsync);
        private readonly Func<Task<MediaKeyHelper.MediaCommandSendOutcome>> _mediaPreviousTrackCommand = ResolveMediaCommand(mediaPreviousTrackCommandDetailedAsync, mediaPreviousTrackCommandAsync, mediaPreviousTrackCommand, MediaKeyHelper.TryPressPreviousTrackDetailedAsync);
        ...
        private readonly record struct PendingMediaOverlayCapture(
            ...
            Func<Task<MediaKeyHelper.MediaCommandSendOutcome>> SendCommandAsync,
            int RequestVersion,
            string Source);
        ...
        private void StartMediaCommand(MediaOverlayCommand command, Func<Task<MediaKeyHelper.MediaCommandSendOutcome>> sendCommandAsync, string source)
        {
            int requestVersion = Interlocked.Increment(ref _latestMediaOverlayRequestVersion);
            if (Interlocked.CompareExchange(ref _mediaOverlayCaptureInFlight, 1, 0) != 0)
            {
        ...
        private async Task SendMediaCommandWithoutCaptureAsync(MediaOverlayCommand command, Func<Task<MediaKeyHelper.MediaCommandSendOutcome>> sendCommandAsync, int requestVersion, string source)
        {
            long started = Stopwatch.GetTimestamp();
            MediaKeyHelper.MediaCommandSendOutcome sendOutcome = await SendMediaCommandSerializedAsync(command, sendCommandAsync).ConfigureAwait(false);
            if (!sendOutcome.Sent)
            {
        ...
        private async Task ShowMediaOverlayAsync(MediaOverlayCommand command, Func<Task<MediaKeyHelper.MediaCommandSendOutcome>> sendCommandAsync, int requestVersion, string source)
        {
            long started = Stopwatch.GetTimestamp();
            ...
            try
            {
                MediaOverlayCommandResult commandResult = await mediaOverlayCommands.SendWithDetailedResultAsync(
                    ...
                    async () =>
                    {
                        sendOutcome = await SendMediaCommandSerializedAsync(command, sendCommandAsync).ConfigureAwait(false);
        ...
        private async Task<MediaKeyHelper.MediaCommandSendOutcome> SendMediaCommandSerializedAsync(MediaOverlayCommand command, Func<Task<MediaKeyHelper.MediaCommandSendOutcome>> sendCommandAsync)
        {
            ...
            try
            {
                try
                {
                    MediaKeyHelper.MediaCommandSendOutcome outcome = await sendCommandAsync().ConfigureAwait(false);
                    if (!outcome.Sent)
                    {
                        logger.Warning(
                            "MediaOverlayHelper",
                ...
                catch (Exception ex)
                {
                    logger.Warning(
                        ...
                        nameof(SendMediaCommandSerializedAsync),
                        ex);
        ...
        private void QueuePendingMediaOverlayCapture(MediaOverlayCommand command, Func<Task<MediaKeyHelper.MediaCommandSendOutcome>> sendCommandAsync, int requestVersion, string source)
        {
            ...
            MediaKeyHelper.MediaCommandSendOutcome outcome)
        {
            ...
            details["sendFocusedProcessAvailable"] = FormatBool(outcome.FocusedProcessAvailable);
            details["sendFocusedCandidateFound"] = FormatBool(outcome.FocusedCandidateFound);
            details["sendFocusedCandidateAttempted"] = FormatBool(outcome.FocusedCandidateAttempted);
            details["sendFocusedTargetRejected"] = FormatBool(outcome.FocusedTargetRejected);
            details["sendNoFocusedMatch"] = FormatBool(outcome.NoFocusedMatch);
            ...
            if (!string.IsNullOrWhiteSpace(outcome.FocusedProcessName))
            {
                details["sendFocusedProcess"] = LogPrivacy.Label(outcome.FocusedProcessName);
            }
        ...
        private static Func<Task<MediaKeyHelper.MediaCommandSendOutcome>> ResolveMediaCommand(
            Func<Task<MediaKeyHelper.MediaCommandSendOutcome>>? detailedAsyncCommand,
            Func<Task<bool>>? asyncCommand,
            Func<bool>? syncCommand,
            Func<Task<MediaKeyHelper.MediaCommandSendOutcome>> defaultCommand)
        {
            if (detailedAsyncCommand != null)
            {
                return detailedAsyncCommand;
            }

            if (asyncCommand != null)
            {
                return async () => MediaKeyHelper.MediaCommandSendOutcome.FromDelegate(await asyncCommand().ConfigureAwait(false));
            }

            return syncCommand == null
                ? defaultCommand
                : () => Task.FromResult(MediaKeyHelper.MediaCommandSendOutcome.FromDelegate(syncCommand()));
        }
