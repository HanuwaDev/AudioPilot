...
- **Miscellaneous**: BluetoothReconnectEnabled, **PreserveAudioLevels**, DeviceReferenceFileMode, LogLevel, RedactLogContent, AutoSaveEnabled, ScheduleTimeZoneId
...
`theme`, `auto-save-enabled`, `run-at-startup`, `preserve-audio-levels`, `auto-scroll-to-mixer-on-restore`, `generate-device-reference-file`, `schedule-timezone` (Windows time zone ID, e.g., `Pacific Standard Time` or `UTC`)
