...
namespace AudioPilot.Services.Internal
{
    internal sealed class HotkeyDispatchCoordinator(Logger logger)
    {
        ...
        private readonly Func<long> _utcNowTicksProvider = static () => DateTime.UtcNow.Ticks;
        ...
        internal HotkeyDispatchCoordinator(Logger logger, Func<long> utcNowTicksProvider)
            : this(logger)
        {
            _utcNowTicksProvider = utcNowTicksProvider;
        }
        ...
        public void ExecuteCallback(int hotkeyId, string description, Action? callback, long? enqueueTimestamp = null)
        {
            ...
            long last = _debounceTimestamps.GetOrAdd(hotkeyId, 0);

            if (now - last < DebounceTicks)
            {
                return;
            }

            _debounceTimestamps[hotkeyId] = now;
