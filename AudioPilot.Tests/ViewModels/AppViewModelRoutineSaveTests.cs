            ...
            try
            {
                ...
                Assert.Collection(
                    ...
                    first =>
                    {
                        ...
                        Assert.True(first.TriggerOnAppStart);
                        Assert.Equal(@"C:\Apps\Discord\Discord.exe", first.TriggerAppPath);
                        Assert.True(first.SwitchOutputPerApp);
                        Assert.Equal(400, first.ExecutionDelayMs);
                        Assert.Equal(15, first.CooldownSeconds);
                        Assert.Equal(900, first.TriggerAppStableForMs);
                    },
                    second =>
                    {
