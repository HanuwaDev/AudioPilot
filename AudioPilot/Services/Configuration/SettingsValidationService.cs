    ...
    public static class SettingsValidationService
    {
        ...
        private static void AddInvalidRoutineWarnings(Settings settings, List<SettingsDiagnostic> warnings)
        {
            for (int index = 0; index < settings.Routines.Items.Count; index++)
            {
                ...
                if (!hasOutputTarget && !hasInputTarget)
                {
                    warnings.Add(new SettingsDiagnostic(
                        Code: $"invalid-routine-target-{index}",
                    Message: $"{routineLabel} must target at least one output device or input device.",
                    SuggestedAction: "Choose an output device, an input device, or both before saving routines. Volume targets are optional extras that apply when the routine runs."));
                }
                ...
                if (routine.RestorePreviousAudioOnDeactivate && !routine.IsStatefulTrigger)
                {
                    ...
                    SuggestedAction: "Use an application startup or Steam Big Picture trigger, or turn off restore on exit."));
                }
                ...
                if (routine.TriggerKind == RoutineTriggerKind.AppStartup && !RoutineTriggerPathHelper.LooksLikeSupportedStartupTarget(routine.TriggerAppPath))
                {
                    warnings.Add(new SettingsDiagnostic(
                        Code: $"invalid-routine-trigger-app-path-{index}",
                    Message: $"{routineLabel} must use a full .exe path or packaged app AUMID for application startup triggers.",
                    SuggestedAction: "Choose an executable with Browse, pick a packaged app, or enter a full .exe path or packaged app AUMID."));
                }

                if (routine.SwitchOutputPerApp && (routine.TriggerKind != RoutineTriggerKind.AppStartup || (!hasOutputTarget && !hasInputTarget)))
                {
                    warnings.Add(new SettingsDiagnostic(
                        Code: $"invalid-routine-app-audio-only-{index}",
                        Message: $"{routineLabel} can switch application audio only for application startup routines with at least one target.",
                        SuggestedAction: "Enable the application startup trigger and choose an output device, an input device, or both, or turn off application audio routing."));
                }
                ...
                if (routine.Enabled && routine.TriggerKind == RoutineTriggerKind.Hotkey && string.IsNullOrWhiteSpace(routine.Hotkey))
                {
                    ...
                    SuggestedAction: "Set a routine hotkey or switch the trigger mode to application startup."));
                }
        ...
        private static List<AudioRoutine> NormalizeRoutines(IEnumerable<AudioRoutine>? routines)
        {
            ...
            foreach (AudioRoutine? routine in routines)
            {
                ...
                bool triggerOnAppStart = triggerKind == RoutineTriggerKind.AppStartup;
                bool isStatefulTrigger = triggerKind is RoutineTriggerKind.AppStartup or RoutineTriggerKind.SteamBigPicture;
                ...
                normalized.Add(new AudioRoutine
                {
                    ...
                    TriggerAppPath = triggerOnAppStart ? RoutineTriggerPathHelper.NormalizeTriggerTarget(routine.TriggerAppPath) : string.Empty,
                    SwitchOutputPerApp = triggerOnAppStart && (!string.IsNullOrWhiteSpace(routine.OutputDeviceId) || !string.IsNullOrWhiteSpace(routine.InputDeviceId)) && routine.SwitchOutputPerApp,
                    ...
                    ExecutionDelayMs = Math.Max(0, routine.ExecutionDelayMs),
                    CooldownSeconds = Math.Max(0, routine.CooldownSeconds),
                    TriggerAppStableForMs = triggerOnAppStart ? Math.Max(0, routine.TriggerAppStableForMs) : 0,
        ...
        private static RoutineTriggerKind NormalizeRoutineTriggerKind(AudioRoutine routine)
        {
            ...
            if (routine.TriggerKind == RoutineTriggerKind.AppStartup || routine.TriggerKind == RoutineTriggerKind.AudioPilotStartup || routine.TriggerKind == RoutineTriggerKind.SteamBigPicture || routine.TriggerKind == RoutineTriggerKind.DeviceChange)
            {
                return routine.TriggerKind;
            }

            return routine.TriggerOnAppStart
