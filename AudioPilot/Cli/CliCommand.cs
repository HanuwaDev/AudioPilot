    ...
    public sealed partial class CliCommand
    {
        ...
        public static bool TryParse(string[] args, out CliCommand command, out string? error)
        {
            ...
            error = CliSuggestionHelper.BuildUnknownValueError(
                "command",
                args[0],
                ["completion", "help", "version", "show", "hide", "refresh", "diagnostics", "media", "mute", "listen", "volume", "routine", "config", "runtime", "wait", "startup", "status", "devices", "cycle", "switch"],
                "completion|show|hide|refresh|diagnostics|media|mute|listen|volume|routine|config|runtime|wait|startup|status|devices|cycle|switch|help|version");
            return false;
        }
