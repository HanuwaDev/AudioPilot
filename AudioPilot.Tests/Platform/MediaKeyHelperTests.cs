...
public sealed class MediaKeyHelperTests
{
    ...
    public void TryPressNextTrackDetailed_HonorsExplicitFallbackSuppression_WhenDetailedCommandRequestsIt()
    {
        try
        {
            ...
            Assert.False(outcome.FocusedTargetRejected);
            Assert.False(outcome.FocusedCandidateFound);
            Assert.False(outcome.FocusedCandidateAttempted);
            Assert.False(outcome.UsedSendInputFallback);
    ...
    [Fact]
    public void TryPressNextTrackDetailed_UsesNativeFallback_WhenFocusedProcessHasNoSessionMatch()
    {
        try
        {
            MediaKeyHelper.DetailedSystemMediaCommandOverrideForTests = static _ => new MediaKeyHelper.MediaCommandSendOutcome(
            ...
            Assert.True(outcome.NoFocusedMatch);
            Assert.True(outcome.FocusedProcessAvailable);
    ...
    [Fact]
    public void TryPressPlayPauseDetailed_UsesNativeFallback_WhenForegroundProcessUnavailable()
    {
        try
        {
            MediaKeyHelper.DetailedSystemMediaCommandOverrideForTests = static _ => new MediaKeyHelper.MediaCommandSendOutcome(
                ...
                FailureReason: "no-foreground-process-identity");
            MediaKeyHelper.SendInputOverrideForTests = static _ => (2u, 0);
            ...
            Assert.True(outcome.Sent);
            Assert.Equal(MediaKeyHelper.MediaCommandRouteKind.SendInputFallback, outcome.Route);
            Assert.False(outcome.FocusedProcessAvailable);
            Assert.False(outcome.NoFocusedMatch);
            Assert.True(outcome.UsedSendInputFallback);
        }
        finally
        {
